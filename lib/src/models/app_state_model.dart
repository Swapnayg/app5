...
class AppStateModel extends Model {
  static final AppStateModel _appStateModel = AppStateModel();
  ...
  CartModel shoppingCart =
      CartModel(cartContents: [], cartTotals: CartTotals());
  int count = 0;
  ...
  fetchAllBlocks() async {
    ...
    if (storedCurrency!.isNotEmpty && storedCurrency != '0') {
  ...
  Future<void> clearCart() async {
    shoppingCart = CartModel(cartContents: [], cartTotals: CartTotals());
    notifyListeners();
    final response = await apiProvider
  ...
  setSubCategories(int id) {
    subCategories =
        blocks!.categories.where((cat) => cat.parent == id).toList();
  ...
  loadMore() async {
    ...
    notifyListeners();
    productsPage[productsFilter['id']] =
        (productsPage[productsFilter['id']]! + 1);
    productsFilter['page'] = productsPage[productsFilter['id']].toString();
